...
CREATE OR REPLACE PACKAGE aunt AS
	PROCEDURE FindAunt(S IN VARCHAR2);
END aunt;
/

CREATE OR REPLACE PACKAGE BODY aunt AS
	PROCEDURE FindAunt(S IN VARCHAR2)
	IS
			...
			FOR R2 IN (SELECT parentName, childName FROM Parent1 NATURAL JOIN Parent2@site1) LOOP
				...
				if A = D Then
					FOR R3 IN (SELECT parentName, childName FROM Parent1 NATURAL JOIN Parent2@site1) LOOP
						...
						F := R3.childName;
						if C = E and A <> F Then
							FOR R4 IN (select name from Female1 union select * from Female2@site1) LOOP
								G := R4.name;
								if G = F then
